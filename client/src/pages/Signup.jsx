...
function Signup() {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      setLoading(true)
      const res = await axios.post(`${baseurl}/api/register`, { username, email, password },{withCredentials:true});
  ...
  return (
    <div className='w-screen h-screen flex justify-center items-center'>
      <div className='w-full max-w-2xl border shadow-md shadow-gray-500 rounded-sm p-2'>
        <h1 className='text-2xl lg:3xl text-center'>Signup</h1>
        <form className='flex flex-col gap-y-2 p-2' onSubmit={handleSignup}>

          <label>username</label>
          <input type="text" placeholder='enter username' value={username} onChange={(e) => setUsername(e.target.value)} className='p-2 outline-none border-2 border-gray-500' required />
          ...
          <button className='bg-blue-500 text-white flex items-center  justify-center gap-x-1 p-2'>
            Signup {loading && <Loader className='animate-spin' />}
          </button>
        </form>
